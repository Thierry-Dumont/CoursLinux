...
Mais \ttt{rsync} a plein d'options:

\begin{itemize}
\item On peut déjà ajouter \ttt{-a} (archive):
...
\begin{enumerate}
  \item \com{mount /mnt}\footnote{Je suppose qu'il y a la bonne ligne
  ...
  \item Synchroniser avec \ttt{rsync}.  La commande est:
    ...
    \begin{itemize}
      \item \ttt{-a}: \og archive\fg{}: conserver les droits  les
        propriétaires et les groupes: bref faire une sauvegarde
        exacte.
      \item \ttt{\tirr delete}: effacer dans le répertoire de backup (ici
        \ttt{/mnt}) tout ce qui n'est pas dans la source.
    \end{itemize}
    ...
    Options à considérer, en tout cas pour la mise au point:
     ...
     \textbf{Ne pas oublier les `''/'' \`A LA FIN pour les
       répertoires:  \ttt{/home/}, \ttt{/mnt/home/}.}
...
\begin{itemize}
  ...
  \item  \ttt{\&>>/var/log/sauv.log}. On se souvient de la
    ...
    n'écrase pas le contenu de  \ttt{/var/log/sauv.log}).
\end{itemize}
...
les chaînes de caractères \ttt{/home/} et \ttt{/etc/}.  \`A la
...
\end{verbatim}
  \end{boxedminipage}
  \end{center}
Et voilà: \ttt{d} contenant succesivement \ttt{/home/} et \ttt{/etc/},
